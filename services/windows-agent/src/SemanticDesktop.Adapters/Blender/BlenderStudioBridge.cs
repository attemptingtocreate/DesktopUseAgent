...
public sealed class BlenderStudioBridge : IBlenderBridge
{
    private readonly BlenderBridgeOptions _options;
    private readonly byte[] _tokenBytes;
    private readonly ConcurrentDictionary<string, BlenderBridgeSession> _sessions = new(StringComparer.Ordinal);
    ...
    public BlenderStudioBridge(BlenderBridgeOptions options)
    {
    ...
    public BlenderBridgeHealth GetHealth()
    {
        ...
        var sessions = _sessions.Values
            ...
            .Select(s => new BlenderBridgeSessionInfo
            {
                ...
                BlenderVersion = s.BlenderVersion,
                FileName = s.FileName,
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        lock (_listenerGate)
        {
            ...
            _listener.Prefixes.Add($"http://{_options.Host}:{_actualPort}/");
            try
            {
                _listener.Start();
                _startError = null;
            ...
            catch (HttpListenerException ex)
            {
                _listener.Close();
                _listener = null;
                _startError = $"Blender bridge port {_actualPort} is unavailable: {ex.Message}";
                IsListening = false;
                return Task.CompletedTask;
            }
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (loop is not null)
        {
            try { await loop.ConfigureAwait(false); } catch { /* expected */ }
        }

        if (cleanupLoop is not null)
        {
            try { await cleanupLoop.ConfigureAwait(false); } catch { /* expected */ }
        }

        cts?.Dispose();
        FailAllPending(ErrorCodes.Cancelled, "Blender bridge stopped.");
    }
        ...
        CancellationToken cancellationToken)
    {
        if (!IsListening)
        {
            return BlenderCommandResult.Fail(ErrorCodes.AdapterUnavailable, "Blender bridge is not listening.");
        }

        if (!BlenderBridgeOperations.Allowlist.Contains(operation))
        {
            return BlenderCommandResult.Fail(ErrorCodes.Unsupported, $"Operation '{operation}' is not allowed.");
        }

        var session = ResolveSession(sessionId);
        if (session is null)
        {
            return BlenderCommandResult.Fail(ErrorCodes.AdapterUnavailable, "No connected Blender add-on session.");
        }

        if (!session.PollingEnabled)
        {
            return BlenderCommandResult.Fail(ErrorCodes.AdapterUnavailable, "Blender add-on polling is not enabled.");
        }

        var command = new BlenderBridgeCommand
        {
        ...
        TaskCompletionSource<BlenderCommandResult> tcs;
        lock (session.Gate)
        {
            if (session.OutstandingCommand is not null)
            {
                return BlenderCommandResult.Fail(ErrorCodes.InvalidArgument, "Blender session already has an outstanding command.");
            }

            tcs = new TaskCompletionSource<BlenderCommandResult>(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        await using var reg = timeoutCts.Token.Register(() =>
        {
            ...
            tcs.TrySetResult(BlenderCommandResult.Fail(ErrorCodes.Timeout, "Timed out waiting for Blender add-on response."));
        });
    ...
    private BlenderBridgeSession? ResolveSession(string? sessionId)
    {
        var now = DateTimeOffset.UtcNow;
        if (!string.IsNullOrWhiteSpace(sessionId))
        {
            if (!BlenderBridgeSecurity.TryNormalizeSessionId(sessionId, out var normalized, out _))
            {
                return null;
            }
    ...
    private void PurgeExpiredSessions(DateTimeOffset now)
    {
        foreach (var pair in _sessions)
        {
            ...
            tcs?.TrySetResult(BlenderCommandResult.Fail(ErrorCodes.Timeout, "Blender session expired."));
        }
    }
    ...
    private async Task HandleHealthAsync(HttpListenerContext ctx)
    {
        ...
        await WriteJsonAsync(ctx, HttpStatusCode.OK, new
        {
            ...
            addonConnected = health.AddonConnected,
            port = health.Port,
    ...
    private async Task HandleRegisterAsync(HttpListenerContext ctx, byte[] body)
    {
        ...
        if (!BlenderBridgeSecurity.TryNormalizeSessionId(sessionIdRaw, out var sessionId, out var sessionError))
        {
            await WriteJsonAsync(ctx, HttpStatusCode.BadRequest, new { ok = false, error = sessionError }).ConfigureAwait(false);
            return;
        }

        var session = _sessions.GetOrAdd(sessionId!, id => new BlenderBridgeSession { SessionId = id });
        lock (session.Gate)
        {
            session.BlenderVersion = TruncateMetadata(GetString(root, "blenderVersion"));
            session.FileName = TruncateMetadata(GetString(root, "fileName"));
    ...
    private async Task HandleHeartbeatAsync(HttpListenerContext ctx, byte[] body)
    {
        ...
        {
            session.LastSeenUtc = DateTimeOffset.UtcNow;
            session.FileName = TruncateMetadata(GetString(doc.RootElement, "fileName")) ?? session.FileName;
        }
    ...
    private async Task HandlePollAsync(HttpListenerContext ctx, CancellationToken cancellationToken)
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            ...
            lock (session.Gate)
            {
                ...
                {
                    session.OutstandingCommand = null;
                    session.PendingResult?.TrySetResult(BlenderCommandResult.Fail(ErrorCodes.Timeout, "Command expired before add-on polled."));
                    session.PendingResult = null;
                    command = null;
                }
            }
        ...
        lock (session.Gate)
        {
            ...
            if (ok)
            {
                object? data = root.TryGetProperty("data", out var dataEl)
                    ...
                    : null;
                result = BlenderCommandResult.Success(data);
            }
            else
            {
                var message = BlenderBridgeSecurity.SanitizeErrorMessage(
                    GetString(root, "error") ?? GetString(root, "message"));
                result = BlenderCommandResult.Fail(ErrorCodes.AdapterFailed, message);
            }
    ...
    private static string? TruncateMetadata(string? value) =>
        string.IsNullOrWhiteSpace(value) ? value : value.Length <= 256 ? value : value[..256];
    ...
    private void FailAllPending(string code, string message)
    {
        foreach (var session in _sessions.Values)
        {
            ...
            tcs?.TrySetResult(BlenderCommandResult.Fail(code, message));
        }
    }
}
