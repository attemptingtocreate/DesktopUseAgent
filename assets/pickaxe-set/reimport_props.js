...
const src = "-- Props only: roll/side/pedestal/signs\nreturn " + toLua(packages) + "\n";
...
function push(instanceId, source) {
  return new Promise((resolve, reject) => {
    ...
    s.on("data", (d) => {
      ...
      if (buf.includes("\n")) {
        clearTimeout(t);
        console.log(buf.trim().slice(0, 400));
        s.end();
  ...
  return new Promise((resolve, reject) => {
    ...
    s.on("data", (d) => {
      ...
      if (buf.includes("\n")) {
        clearTimeout(t);
        const line = buf.trim();
        console.log(method, line.slice(0, 300));
        s.end();
...
(async () => {
  const created = await rpc("roblox.create_instance", {
    className: "ModuleScript",
    name: "PropsOnlyMesh",
    parentPath: "ServerStorage",
  });
  const instId = created.data?.id || created.data?.instanceId;
  if (!instId) throw new Error("no instance id: " + JSON.stringify(created));
  await push(instId, src);
  console.log("DONE", instId);
})().catch((e) => {
  console.error(e);
  process.exit(1);
});
